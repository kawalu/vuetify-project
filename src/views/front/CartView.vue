        ...
        <VTable>
          ...
          <tbody>
            <tr v-for="item in cart" :key="item._id" :class="{ 'bg-red-lighten-5': !item.product.sell }">
              ...
              <td>
                <VBtn color="primary" variant="icon" icon="mdi-plus" @click="updateCart(item.product._id, 1)"></VBtn>
                {{ item.quantity }}
                <VBtn color="primary" variant="icon" icon="mdi-minus" @click="updateCart(item.product._id, -1)"></VBtn>
              </td>
              <td>{{ item.quantity * item.product.price }}</td>
              <td>
                <VBtn color="red" variant="icon" icon="mdi-delete"
